...
export function ERPImportProfilesCard({ selectedCompanyId }: ERPImportProfilesCardProps) {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const parseAliases = (s: string) => s.split(",").map(x => x.strip ? x.strip() : x.trim()).filter(Boolean);
    ...
    const payload: ImportProfileCreate = {
      ...
      validation_rules: formModule === "sales" ? {
        module: "sales",
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl border border-slate-200 max-w-xl w-full p-6 space-y-4 overflow-y-auto max-h-[90vh]">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-900">Add Custom Import Profile</h3>
              <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-600 text-lg cursor-pointer">
                ×
              </button>
            </div>
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Profile Name *</label>
                  <input
                    type="text"
                    required
                    value={formName}
                    ...
                    className="w-full px-3 py-2 border border-slate-200 rounded-md focus:outline-none focus:border-blue-600"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-slate-200 rounded-md focus:outline-none focus:border-blue-600 bg-white"
                  >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Provider Label</label>
                  <input
                    type="text"
                    value={formProvider}
                ...
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Source Format</label>
                  <select
                    value={formFormat}
              ...
              <div className="pt-2 border-t border-slate-100">
                ...
                <div className="space-y-2 font-mono text-[11px]">
                  <div>
                    <label className="block font-semibold text-slate-600 mb-0.5">PIN Aliases</label>
                    <input type="text" value={pinAlias} onChange={(e) => setPinAlias(e.target.value)} className="w-full px-2.5 py-1.5 border border-slate-200 rounded-md" />
                  </div>
                  <div>
                    <label className="block font-semibold text-slate-600 mb-0.5">Partner Name Aliases</label>
                    <input type="text" value={partnerAlias} onChange={(e) => setPartnerAlias(e.target.value)} className="w-full px-2.5 py-1.5 border border-slate-200 rounded-md" />
                  </div>
                  <div>
                    <label className="block font-semibold text-slate-600 mb-0.5">Invoice Number Aliases</label>
                    <input type="text" value={invNumAlias} onChange={(e) => setInvNumAlias(e.target.value)} className="w-full px-2.5 py-1.5 border border-slate-200 rounded-md" />
                  </div>
                  <div>
                    <label className="block font-semibold text-slate-600 mb-0.5">Invoice Date Aliases</label>
                    <input type="text" value={invDateAlias} onChange={(e) => setInvDateAlias(e.target.value)} className="w-full px-2.5 py-1.5 border border-slate-200 rounded-md" />
                  </div>
                  <div>
                    <label className="block font-semibold text-slate-600 mb-0.5">CU/ETR Number Aliases</label>
                    <input type="text" value={cuNumAlias} onChange={(e) => setCuNumAlias(e.target.value)} className="w-full px-2.5 py-1.5 border border-slate-200 rounded-md" />
                  </div>
                  <div>
                    <label className="block font-semibold text-slate-600 mb-0.5">VAT Group / Tax Rate Aliases</label>
                    <input type="text" value={vatGroupAlias} onChange={(e) => setVatGroupAlias(e.target.value)} className="w-full px-2.5 py-1.5 border border-slate-200 rounded-md" />
                  </div>
                  <div>
                    <label className="block font-semibold text-slate-600 mb-0.5">Base Amount Aliases</label>
                    <input type="text" value={baseAmtAlias} onChange={(e) => setBaseAmtAlias(e.target.value)} className="w-full px-2.5 py-1.5 border border-slate-200 rounded-md" />
                  </div>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end gap-2">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="px-4 py-2 bg-[#0e1734] hover:bg-[#16224c] text-white rounded-lg font-semibold transition-colors flex items-center gap-2 cursor-pointer disabled:opacity-50"
                >
