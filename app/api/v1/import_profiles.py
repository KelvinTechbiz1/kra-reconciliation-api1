...
def create_import_profile(
    ...
    if company_id is None:
        ...
        comp = db.query(Company).order_by(Company.id.asc()).first()
        if not comp:
            raise HTTPException(status_code=400, detail="No company found for current user.")
        company_id = comp.id
...
def update_import_profile(
    ...
    if not company_id:
        from app.models.company import Company
        comp = db.query(Company).order_by(Company.id.asc()).first()
        company_id = comp.id if comp else 0
...
def clone_import_profile(
    ...
    if not company_id:
        ...
        comp = db.query(Company).order_by(Company.id.asc()).first()
        if not comp:
            raise HTTPException(status_code=400, detail="No company context available.")
        company_id = comp.id
